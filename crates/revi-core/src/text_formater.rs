...
fn fill_rest_of_screen(
	formated_text: &mut String,
	filler: char,
	width: usize,
	height: usize,
) {
...
fn format_line(
	...
	let len = line.len();
	if let Some(s) =
		line.get(offset.min(len)..len.min(width.saturating_add(offset)))
	{
...
fn test_format_screen_with_filler() {
	...
	let offset_x = 15;
	let offset_y = 0;
	let view = buffer.on_screen(offset_y, height + offset_y);
	let new = format_screen(view.as_str(), offset_x, width, height);
	eprint!("{:?}", buffer);
	eprint!("{}", new);
	assert_eq!(
        new,
