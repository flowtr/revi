...
impl LuaUserData for ReVi {
	fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
		methods.add_method("cursor_pos", |_, revi, ()| {
			Ok(revi.cursor_position_u16())
		});
...
impl LuaUserData for Mapper {
	...
	fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
		...
		methods.add_method_mut(
			...
			|_,
			 mapper,
			 (mode, keys, commands): (Mode, String, Vec<Command>)| {
				eprintln!(
					"Mode: {:?}, Keys: {:?}, Command: {:?}",
					mode, keys, commands
				);
...
pub fn initialize(lua: &Lua) -> Result<()> {
	...
	let mode = lua.create_function(|_, mode: String| {
	...
	let command = lua.create_function(|_, command: String| {
