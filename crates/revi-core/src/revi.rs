...
impl ReVi {
	...
	pub fn new(files: &[String]) -> Rc<RefCell<Self>> {
		...
		let command_bar =
			Window::new(w, 1, cbuffer).with_position((0, h + 2).into());
	...
	pub fn enter_command_mode(&mut self) {
		...
		*self.mode_mut() = Mode::Insert;
		let end = self.buffers[0].borrow().len_chars();
		let last_char =
			self.buffers[0].borrow().get_char(end.saturating_sub(1));
		if last_char == Some('\n') || last_char.is_none() {
	...
	pub fn execute(&mut self, count: usize, commands: &[Command]) {
	...
	fn run_command_line(&mut self, command: &str) {
		...
		match items.remove(0) {
			num if num.parse::<usize>().ok().is_some() => {
				let x =
					self.windows[self.last_focused].cursor_file().as_usize_x();
				let max_y =
					self.windows[self.last_focused].buffer().len_lines();
			...
			"line" => {
				let line_number =
					self.windows[self.last_focused].cursor_file().as_usize_y();
				let text =
					self.windows[self.last_focused].buffer().line(line_number);
			...
			"b" if !items.is_empty() => {
				if let Some(i) =
					items.first().and_then(|i| i.parse::<usize>().ok())
				{
					let buffer = self.buffers.get(i).cloned();
			...
			"set" if !items.is_empty() => {
				match items.first().copied().unwrap_or_default() {
					"number" => self.windows[self.last_focused]
						.set_number(LineNumberKind::AbsoluteNumber),
					"relativenumber" => self.windows[self.last_focused]
						.set_number(LineNumberKind::RelativeNumber),
					"nonumber" | "norelativenumber" => self.windows
						[self.last_focused]
						.set_number(LineNumberKind::None),
...
impl revi_ui::Display<String> for ReVi {
	...
	fn cursor(
		&self,
		mut func: impl FnMut(u16, u16, Option<revi_ui::CursorShape>),
	) {
